...
public class IBatisDemoESXServer {
	...
	public static void main(String[] args) throws IOException{
		...
		ESXServer cViewModel = new ESXServer();
		cViewModel.setUuid(Rpid.generate());
		try {
			sqlMap.insert("insertesxserver", cViewModel);
		...
		List<ESXServer> list;
		try {
			list = sqlMap.queryForList("checkesxserver");
			for (ESXServer clusterViewModel : list) {
